...
namespace school_management_system_model.Classes
{
    internal class TuitionFeeSetup
    {
        ...
        public DataTable loadRecords()
        {
            ...
            var da = new MySqlDataAdapter("select * from tuition_fee_setup", con);
        ...
        public void addRecords()
        {
            var con = new MySqlConnection(connection.con());
            con.Open();
            var cmd = new MySqlCommand("insert into tuition_fee_setup(category, campus, first_year, second_year, third_year, fourth_year, description) " +
        ...
        public void editRecords()
        {
            var con = new MySqlConnection(connection.con());
            con.Open();
            var cmd = new MySqlCommand("update tuition_fee_setup set category=@1, campus=@2, first_year=@3, " +
                "second_year=@4, third_year=@5, fourth_year=@6, description=@7 where id='"+ id +"'", con);
            cmd.Parameters.AddWithValue("@1", category);
            cmd.Parameters.AddWithValue("@2", campus);
            cmd.Parameters.AddWithValue("@3", first_year);
            cmd.Parameters.AddWithValue("@4", second_year);
        ...
        public DataTable deleteRecords(int id)
        {
            ...
            da.SelectCommand = new MySqlCommand("delete from tuition_fee_setup where id='"+ id +"'", con);
            var dt = new DataTable();
            da.Fill(dt);
            return dt;
        }
        ...
        {
            var con = new MySqlConnection( connection.con());
            var da = new MySqlDataAdapter("select * from tuition_fee_setup where concat(description) like '%" + search + "%'", con);
            var dt = new DataTable();
            da.Fill(dt);
            return dt;
        }
