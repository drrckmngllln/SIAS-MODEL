...
{
    internal class LabFeeRepository : IGenericRepository<LabFee>
    {
        LevelsRepository _levelRepo = new LevelsRepository();
        CampusRepository _campusRepo = new CampusRepository();
        public async Task AddRecords(LabFee entity)
        {
        ...
        public async Task DeleteRecords(LabFee entity)
        {
            using (var con = new MySqlConnection(connection.con()))
            {
                await con.OpenAsync();
                var cmd = new MySqlCommand("delete from lab_fee_setup where id='" + entity.id + "'", con);
                await cmd.ExecuteNonQueryAsync();
                await con.CloseAsync();
            }
        }

        public async Task<IReadOnlyList<LabFee>> GetAllAsync()
        {
            var list = new List<LabFee>();
        ...
        public async Task<LabFee> GetByIdAsync(int id)
        {
            var labFee = new LabFee();
            using (var con = new MySqlConnection(connection.con()))
            {
                await con.OpenAsync();
                var sql = "select * from lab_fee_setup where id='" + id + "'";
                using (var cmd = new MySqlCommand(sql, con))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            var levels = await _campusRepo.GetByIdAsync(reader.GetInt32("level_id"));
        ...
        public async Task UpdateRecords(LabFee entity)
        {
            using (var con = new MySqlConnection(connection.con()))
            {
                await con.OpenAsync();
